...
def plot_denoised_counts(counts, denoised_counts, pos, checkpoint_dir):
    ...
    c_max = np.hstack((counts[:, [405, 1513, 381, 1475, 993]],
                       denoised_counts[:, [405, 1513, 381, 1475, 993]])).max()
    c_min = np.hstack((counts[:, [405, 1513, 381, 1475, 993]],
                       denoised_counts[:, [405, 1513, 381, 1475, 993]])).min()
    counts_norm = (counts[:, [405, 1513, 381, 1475, 993]]  - c_min) / (c_max - c_min)
    denoised_counts_norm = (denoised_counts[:, [405, 1513, 381, 1475, 993]]  - c_min) / (c_max - c_min)

    # Plot normalized counts
    cmap = plt.get_cmap('jet', 11)
    for i, gene in enumerate(["CXCL14", "ATP2B4", "CARTPT", "PCP4", "PCDH17"]):
