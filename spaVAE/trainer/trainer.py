...
class Trainer:
    ...
    def train_epoch(self, epoch):
        ...
        for batch_idx, data in enumerate(self.data_loader):
            self.train_batch(epoch, batch_idx, data)
    ...
    def valid_epoch(self, epoch):
        ...
        with torch.no_grad():
            for batch_idx, data in enumerate(self.valid_data_loader):
                self.valid_batch(epoch, batch_idx, data)
        return self.valid_metrics.result()

    @logged(logger=logger, message_after=log_train_batch)
    def train_batch(self, epoch, batch_idx, data):
        ...
        output = self.model(x, y, raw_y, size_factors, update_beta=True)
        ...
        loss_val = loss.detach() / y.shape[0]
        self.train_metrics.update('loss', loss_val)
        for met in self.metric_ftns:
            self.train_metrics.update(met.__name__, met(output_))
        return loss_val

    def valid_batch(self, epoch, batch_idx, data):
        x, y, raw_y, size_factors = data
        x = x.to(self.device)
        y = y.to(self.device)
        raw_y = raw_y.to(self.device)
        ...
        output = self.model(x, y, raw_y, size_factors)
        loss = self.criterion(output)
        ...
        self.valid_metrics.update('loss', loss.detach() / y.shape[0])
        for met in self.metric_ftns:
            self.valid_metrics.update(met.__name__, met(output_))
